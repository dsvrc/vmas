#  Entry point for the simple_ns tasks.  P-10.1: every arm launches through
#  THIS, with severity supplied from outside the method, so the baselines are
#  bit-for-bit the ones BenchMARL ships.
#
...
#  sys.path[0] rather than the repo root, so `import benchmarl` can resolve to an
...
def _check_import_is_this_checkout() -> None:
    ...
    if missing:
        raise RuntimeError(
            ...
            "benchmarl/environments/__init__.py lists SimpleNsTask in `tasks` "
            "and that SimpleNsTask declares a member per task."
        )
